...
@interface HaishijuHccyViewController ()

@end

@implementation HaishijuHccyViewController
...
- (void)viewDidLoad
{
    ...
    jigouArray=[[NSMutableArray alloc] initWithCapacity:0];
    B01Array=[[NSMutableArray alloc] initWithCapacity:0];
    B02Array=[[NSMutableArray alloc] initWithCapacity:0];
    total=0;
    totalPage=0;
    currentPage=0;
    keyWordsArray=[[NSArray alloc] initWithObjects:@"培训机构名称",nil];
    keyWordsString=[[NSArray alloc] initWithObjects:@"jigou", nil];
    ...
    gzcbDetailViewController=[[HaishijuHccyDetailViewController alloc] init];
}
-(void)viewWillAppear:(BOOL)animated
{
    self.navigationController.navigationBarHidden=YES;
    HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
    NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&Page=%d",1];
    ...
    }else
    {
        ...
        for (NSDictionary *dic in dataList)
        {
            ...
            [jigouArray addObject:[dic objectForKey:@"jigou"]];
            [B01Array addObject:[dic objectForKey:@"B01"]];
            [B02Array addObject:[dic objectForKey:@"B02"]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0)
    {
        ...
        chuanmingLabel.text=@"培训机构名称";
        UILabel *B01Label=[[UILabel alloc] initWithFrame:CGRectMake(240, 10, 40, 40)];
        UILabel *B02Label=[[UILabel alloc] initWithFrame:CGRectMake(280, 10, 40, 40)];
        B01Label.text=@"B01";
        B02Label.text=@"B02";
        B01Label.font=B02Label.font=chuanmingLabel.font=font;
        chuanmingLabel.backgroundColor=B01Label.backgroundColor=B02Label.backgroundColor=[UIColor clearColor];
        chuanmingLabel.textColor=B01Label.textColor=B02Label.textColor=[UIColor blackColor];
        [cell addSubview:chuanmingLabel];
        [cell addSubview:B01Label];
        [cell addSubview:B02Label];
    ...
    }else
    {
        ...
        UILabel *chuanmingLabel=[[UILabel alloc] initWithFrame:CGRectMake(40, 10, 200, 40)];
        chuanmingLabel.text=[jigouArray objectAtIndex:indexPath.row-1];
        UILabel *B01Label=[[UILabel alloc] initWithFrame:CGRectMake(240, 10, 40, 40)];
        UILabel *B02Label=[[UILabel alloc] initWithFrame:CGRectMake(280, 10, 40, 40)];
        B01Label.text=[B01Array objectAtIndex:indexPath.row-1];
        B02Label.text=[B02Array objectAtIndex:indexPath.row-1];
        B01Label.font=B02Label.font=chuanmingLabel.font=font;
        chuanmingLabel.backgroundColor=B01Label.backgroundColor=B02Label.backgroundColor=[UIColor clearColor];
        chuanmingLabel.textColor=B01Label.textColor=B02Label.textColor=[UIColor blueColor];
...
-(IBAction)tapBotAction:(id)sender
{
    ...
    switch ([sender tag]) {
        case 1:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
                [B01Array removeAllObjects];
                [B02Array removeAllObjects];
            }
            NSArray *dataList=[[NSArray alloc] initWithArray:[initDic objectForKey:@"datalist"]];
            NSLog(@"%d",[dataList count]);
            for (NSDictionary *dic in dataList)
        ...
        case 2:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+1];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        case 3:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+2];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        {if (button4.selected==NO) {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+3];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        case 5:
        {
            if (button5.selected==NO)
            {
                ...
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,totalPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
    ...
    }else
    {
        ...
        }else
        {
            ...
            }else
            {
                ...
                button2.selected=button3.selected=button4.selected=button5.selected=NO;
                HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&Page=%d",currentPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
...
- (IBAction)searchAction:(id)sender
{
    ...
    currentPage=1;
        ...
        urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=pxjg&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
    NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
    NSArray *dataList=[[NSArray alloc] initWithArray:[initDic objectForKey:@"datalist"]];
    NSLog(@"%d",[dataList count]);
