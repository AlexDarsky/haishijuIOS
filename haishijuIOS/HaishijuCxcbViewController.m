...
@interface HaishijuCxcbViewController ()

@end

@implementation HaishijuCxcbViewController
@synthesize tableView,customNavBar,serachField,customSearchBar,button1,button2,button3,button4,button5,buttonMore,keySelectBtn,alert,searchFieldString,cxcbDetailViewController;
...
- (void)viewDidLoad
{
    ...
    gongsisArray=[[NSMutableArray alloc] initWithCapacity:0];
    total=0;
    totalPage=0;
    currentPage=0;
    keyWordsArray=[[NSArray alloc] initWithObjects:@"船名",@"公司",@"船籍港", nil];
    keyWordsString=[[NSArray alloc] initWithObjects:@"ChuanMing",@"GongSi",@"ChuanJiGang", nil];
    ...
    cxcbDetailViewController=[[HaishijuCxcbDetailViewController alloc] init];
}
-(void)viewWillAppear:(BOOL)animated
{
    self.navigationController.navigationBarHidden=YES;
    HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
    NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&Page=%d",1];
    ...
    }else
    {
        ...
        for (NSDictionary *dic in dataList)
        {
            ...
            [chuanjigangsArray addObject:[dic objectForKey:@"ChuanJiGang"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0)
    {
        ...
        UILabel *chuanmingLabel=[[UILabel alloc] initWithFrame:CGRectMake(40, 10, 70, 40)];
        chuanmingLabel.text=@"船名";
        UILabel *chuanjigangLabel=[[UILabel alloc] initWithFrame:CGRectMake(180, 10, 210, 40)];
        chuanjigangLabel.text=@"船籍港";;
...
-(IBAction)tapBotAction:(id)sender
{
    ...
    switch ([sender tag]) {
        case 1:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
        ...
        case 2:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+1];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [chuanmingsArray removeAllObjects];
        ...
        case 3:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+2];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [chuanmingsArray removeAllObjects];
        ...
        {if (button4.selected==NO) {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+3];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [chuanmingsArray removeAllObjects];
        ...
        case 5:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,totalPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [chuanmingsArray removeAllObjects];
    ...
    }else
    {
        ...
        }else
        {
            ...
            }else
            {
                ...
                button2.selected=button3.selected=button4.selected=button5.selected=NO;
                HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&Page=%d",currentPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
...
- (IBAction)searchAction:(id)sender
{
    ...
    currentPage=1;
        ...
        urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=cxcb&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
    NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
    NSArray *dataList=[[NSArray alloc] initWithArray:[initDic objectForKey:@"datalist"]];
    NSLog(@"%d",[dataList count]);
...
-(void)pushAction:(id)sender
{
    ...
    [self.navigationController pushViewController:cxcbDetailViewController animated:YES];
    [cxcbDetailViewController loadInfoBySendRequest:[idsArray objectAtIndex:[sender tag]]];
