...
@interface HaishijuNhcyViewController ()

@end

@implementation HaishijuNhcyViewController
...
- (void)viewDidLoad
{
    ...
    diquArray=[[NSMutableArray alloc] initWithCapacity:0];
    total=0;
    totalPage=0;
    currentPage=0;
    keyWordsArray=[[NSArray alloc] initWithObjects:@"培训机构名称",@"所属辖区",@"驾驶岗位",@"轮机岗位",@"基本安全",@"油船",@"载运包装危险货物船舶",@"客船",@"内河游艇操作人员",nil];
    keyWordsString=[[NSArray alloc] initWithObjects:@"jigou",@"diqu",@"jiashigang",@"lunjigang",@"anquan",@"youchuan",@"weixianhuowu",@"kechuan",@"leiheyouting", nil];
    ...
    gzcbDetailViewController=[[HaishijuNhcyDetailViewController alloc] init];
}
-(void)viewWillAppear:(BOOL)animated
{
    self.navigationController.navigationBarHidden=YES;
    HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
    NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&Page=%d",1];
    ...
    }else
    {
        ...
        for (NSDictionary *dic in dataList)
        {
            ...
            [diquArray addObject:[dic objectForKey:@"diqu"]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0)
    {
        ...
        UILabel *B01Label=[[UILabel alloc] initWithFrame:CGRectMake(240, 10, 80, 40)];
        B01Label.text=@"所属辖区";
        B01Label.font=chuanmingLabel.font=font;
        chuanmingLabel.backgroundColor=B01Label.backgroundColor=[UIColor clearColor];
        chuanmingLabel.textColor=B01Label.textColor=[UIColor blackColor];
    ...
    }else
    {
        ...
        UILabel *B01Label=[[UILabel alloc] initWithFrame:CGRectMake(240, 10, 80, 40)];
        B01Label.text=[diquArray objectAtIndex:indexPath.row-1];
        B01Label.font=chuanmingLabel.font=font;
        chuanmingLabel.backgroundColor=B01Label.backgroundColor=[UIColor clearColor];
        chuanmingLabel.textColor=B01Label.textColor=[UIColor blueColor];
...
-(IBAction)tapBotAction:(id)sender
{
    ...
    switch ([sender tag]) {
        case 1:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
                [diquArray removeAllObjects];
            }
            NSArray *dataList=[[NSArray alloc] initWithArray:[initDic objectForKey:@"datalist"]];
            NSLog(@"%d",[dataList count]);
            for (NSDictionary *dic in dataList)
        ...
        case 2:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+1];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        case 3:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+2];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        {if (button4.selected==NO) {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
            NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage+3];
            NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
        ...
        case 5:
        {
            ...
            HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,totalPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
            if ([idsArray count]>0) {
                [idsArray removeAllObjects];
                [jigouArray removeAllObjects];
    ...
    }else
    {
        ...
        }else
        {
            ...
            }else
            {
                ...
                button2.selected=button3.selected=button4.selected=button5.selected=NO;
                HaishijuServerHelper *serverHelper=[HaishijuServerHelper shareHaishijuServerHelper];
                NSString *urlString =[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&Page=%d",currentPage];
                NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
...
- (IBAction)searchAction:(id)sender
{
    ...
    currentPage=1;
        ...
        urlString=[NSString stringWithFormat:@"http://www.gdmsa.gov.cn/android/getDataList.asp?type=lhcbcy&searchField=%@&keyword=%@&Page=%d",self.searchFieldString,self.serachField.text,currentPage];
    NSDictionary *initDic=[[NSDictionary alloc] initWithDictionary:[serverHelper sendRequestByUrl:urlString]];
    NSArray *dataList=[[NSArray alloc] initWithArray:[initDic objectForKey:@"datalist"]];
    NSLog(@"%d",[dataList count]);
